...
help:
	@echo "ZASI v27.0.0 Unified Full-Stack Build & Setup Automation"
	@echo "=========================================================="
	...
	@echo "make test     - Execute full 141-subsystem unit test suite"
	...
	@echo "make run      - Run complete 144-subsystem main.py pipeline"
...
config: setup
	...
	@if [ ! -f config/zasi_config.json ]; then \
		echo '{"version": "25.0.0", "subsystems": 144, "environment": "production", "formal_verification": true, "frontend_port": $(PORT)}' > config/zasi_config.json; \
	fi
	@echo "[✓] Configuration initialized: config/zasi_config.json"

test: config
	@echo "[*] Running full 141-subsystem test suite..."
	@$(PYTHON) -m unittest discover -s tests -q
	@echo "[✓] All 141 test suites passed."
...
install: build
	...
	@echo "[✓] ZASI v27.0.0 installed successfully into environment."
...
all: setup config test build install
	@echo "=========================================================="
	@echo "[SUCCESS] ZASI v27.0.0 Full-Stack Build & Install Complete"
	@echo "=========================================================="
