...
class ZKnowbaseReadContract:
    ...
    def __post_init__(self) -> None:
        ...
        if not base.startswith(("http://", "https://")):
            raise KnowledgeContractError("zknowbase base_url must be http(s)")
        if not key:
    ...
    def headers(self) -> dict[str, str]:
        # zknowbase authenticates service keys with X-API-Key and validates a
        # tenant context on read routes.  Keep both values server-side.
        return {
