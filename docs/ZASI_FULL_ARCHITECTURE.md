...
| `src/control_plane/multimodal.py` | Bounded stdlib adapters parse actual STEP/STL/OBJ bytes and structurally validate PNG/JPEG bytes after quarantine digest verification. | Local source-observation evidence only: measured geometry and image metadata are exposed with provenance; semantic vision, FEA, thermal, material, mass, manufacturing, STT/TTS, anti-replay biometrics, and hardware remain unenabled. |
...
| `src/control_plane/multimodal.py` | Bounded source-backed STEP/STL/OBJ geometry and PNG/JPEG structural observation adapters | Local measured/structurally validated evidence only; no solver, semantic vision model, STT/TTS engine, speaker authentication, or hardware execution is implied. |
