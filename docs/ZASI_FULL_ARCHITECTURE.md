...
(`web/static/app.tsx`) and checked application source in
`web/static/cockpit.tsx`. The historical `web/static/app.jsx` path is retained
as an explicit compatibility re-export so the migration does not silently
discard the existing governed behavior. Accessibility/performance evidence and the additional
...
|---|---|---|
| React 19 + React Router 7 cockpit | Bundled operator UI with a TypeScript root entrypoint, governed overview, assistive JARVIS, Engineering artifact, capability, safety, and MCP views | Implemented reference UI with a preserved JSX compatibility body; graph and browser voice helpers are not capability proof. STL/OBJ/GLB mesh rendering uses the authenticated artifact-content route and source digest; JSON glTF remains evidence-only in the reference viewer. |
...
| `src/control_plane/multimodal.py` | Bounded source-backed STEP/STL/OBJ/glTF 2.0 geometry and PNG/JPEG structural observation adapters | Local measured/structurally validated evidence only; glTF is GLB/embedded-buffer only and mesh-local; no solver, semantic vision model, STT/TTS engine, speaker authentication, or hardware execution is implied. |
...
Tool actions use the same reference repository with a durable queued run,
immutable payload, bounded timeout, worker lease, and idempotency key.
`ActionWorker` invokes only registry-owned handlers. R0/R1 observations may be
drained inline after enqueue for compatibility, while R2-R5 actions remain
queued for a separately governed worker. Lease expiry, timeout, cancellation
during execution, and uncertain side effects become explicit `unknown` states;
only an authenticated, reasoned reconciliation can return an action to the
queue. The installed `zasi-action-worker` command defaults to R0/R1 and does
not activate external writes.
