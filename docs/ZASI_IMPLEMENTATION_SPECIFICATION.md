...
| GitHub Actions run [#33834811669](https://github.com/cvsz/zasi/actions/runs/33834811669) | The Python 3.12 job failed before typecheck/build because the hosted runner used Node 20 for Electron 44.1.0, which declares Node `>=22.12.0`, and the npm audit endpoint returned transient `503 Service Unavailable`; no vulnerability result was produced. The repair branch pins Node 22.14.0 and retries only bounded transient audit failures. | Hosted failure diagnosis and pending repair rerun; not a release gate pass |
