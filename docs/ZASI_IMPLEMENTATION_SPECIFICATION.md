...
Evidence capture date: **2026-09-04 UTC**. The results below distinguish local
...
| `PYTHONWARNINGS=error::ResourceWarning python3 -m unittest discover -s tests -q` on 2026-09-04 UTC | 379 tests passed with 2 documented optional skips after isolating the legacy compatibility listener from the task-owned staging port; no test failures remained | Fresh local full-suite regression; not hosted CI or production evidence |
...
| `npm ci --ignore-scripts && npm test && npm run typecheck && npm run build` on 2026-09-04 UTC under Node 22.22.1 | Lockfile installation completed; frontend structural/runtime tests, strict TypeScript, and Vite production build passed | Fresh local frontend regression; online bulk-advisory auditing is a separate fail-closed gate |
