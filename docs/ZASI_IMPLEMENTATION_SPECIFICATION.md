...
Evidence capture date: **2026-09-05 UTC**. The results below distinguish local
...
The current CI runtime/audit and disposable PostgreSQL rollback hardening is in
signed commit `bf0c9f83ac65d9e7d89c9f2521dd2ad6a1360290`, the response-header
CSP correction is in signed commit `524fb37464b12dd9a2010bf9dad8f76acfe6050a`,
and the retired npm quick-audit fallback repair is in signed commit
`d6ee153df0d6708075f77a600701e4ec8cc65aa3`;
these commits are intended to land in the canonical
`/home/cvsz/zasi` `main` checkout. No new `zasi-*` directory is an
...
The legacy fail-closed boundary repair is in signed commit
`8f338043b8f58d7c7514ec2751826da7a98450bb`; PR [#40](https://github.com/cvsz/zasi/pull/40)
merged that exact head into main at
`63967db5b023dcb92252331ab86592c991e60d68` after the required hosted checks
passed. Persisted session-scope validation is in signed commit
...
| `PYTHONWARNINGS=error::ResourceWarning python3 -m unittest discover -s tests -q` on 2026-09-05 UTC | 379 tests passed with 2 documented optional skips after isolating the legacy compatibility listener from the task-owned staging port; no test failures remained | Fresh local full-suite regression; not hosted CI or production evidence |
| Isolated project environment: `pip-audit --local` on 2026-09-05 UTC | No known vulnerabilities found after installing the declared project and development dependencies into a fresh virtual environment | Fresh project-scoped Python dependency audit; host-wide distro packages are outside scope |
| `npm ci --ignore-scripts && npm test && npm run typecheck && npm run build` on 2026-09-05 UTC under Node 22.22.1 | Lockfile installation completed; frontend structural/runtime tests, strict TypeScript, and Vite production build passed | Fresh local frontend regression; online bulk-advisory auditing is a separate fail-closed gate |
| Fresh isolated wheel/sdist, SBOM, and signature bundle on 2026-09-05 UTC | Wheel and sdist built outside the repository; CycloneDX SBOM generated with resolved project dependencies; three primary artifacts plus `SHA256SUMS` received detached GPG signatures, and the checksum manifest verified successfully | Local artifact integrity/provenance evidence; not hosted release provenance or tag authorization |
...
| `scripts/rollback_drill.py --allow-local-rehearsal --expected-schema-version 11` against the shared PostgreSQL cluster | Fresh dump/encryption/archive validation passed; restore into a random `zasi_rollback_drill_*` database passed schema-11 and integrity checks with archive ownership ignored for the disposable target; source schema/integrity observation remained unchanged; the temporary database and role were removed. The command rejects staging/production profiles and emits no target database or secret. | Fresh local rollback rehearsal evidence only; managed retention/key rotation, staging canary, production cutover, and rollback observation remain open |
...
| Historical `npm audit --omit=dev` hosted distribution evidence | Earlier hosted distribution validation passed after the React Router 7.18.3 upgrade; it does not prove the current workflow revision | Historical hosted dependency evidence; current repair requires a fresh hosted run |
| GitHub Actions run [#33834811669](https://github.com/cvsz/zasi/actions/runs/33834811669) | The Python 3.12 job failed before typecheck/build because the hosted runner used Node 20 for Electron 44.1.0, which declares Node `>=22.12.0`, and the npm audit endpoint returned transient `503 Service Unavailable`; no vulnerability result was produced. | Hosted failure diagnosis; not a release gate pass |
| `scripts/npm_ci_audit.sh` and `scripts/npm_audit_retry.sh` on 2026-09-05 UTC | npm CLI auditing reached a retired quick endpoint, so the implementation now installs with `--no-audit` only to prevent that fallback, then sends the lockfile's production package/version set to the HTTPS bulk-advisory endpoint. The replacement retries only transient failures, validates advisory ranges/severity, and has no offline fallback or vulnerability suppression. | Fresh local audit-wrapper design/test evidence; hosted bulk-endpoint rerun remains required |
| `node --test tests/test_npm_bulk_audit.js` plus `tests.test_npm_audit_retry` | Four bulk-request/response/retry/fail-closed tests and two compatibility-entrypoint tests passed; the bulk request omits dev packages, deduplicates findings, validates semver ranges, and refuses malformed responses | Local audit implementation regression; not a live registry result |
| Pull-request CI dependency review | `actions/dependency-review-action@v4` is now the change-level dependency gate for PRs because the npm quick endpoint is retired and the bulk service was unavailable during the repair run; release workflows still require the direct HTTPS bulk audit and fail closed on unavailable results | Hosted change-level dependency gate; not a full release audit or production approval |
| `docker build --pull=false -t zasi-ci-hardening:20260904 .` | Image build passed under the declared Node 22.14.0 builder; deprecated transitive-package warnings remained non-blocking because no vulnerability result was asserted from them | Fresh local container build; hosted Docker checks remain required |
| Hardened container smoke for `zasi-ci-hardening:20260904` | A root-owned `/app/data` tmpfs was correctly rejected by the private-directory guard. With the disposable tmpfs explicitly owned by UID/GID `10001:10001`, mode `700`, the read-only-rootfs container returned HTTP `200` from `/health/ready`, reached `healthy`, reported schema `11`, and kept external egress, research execution, and physical actuation disabled; the temporary container was removed | Fresh local constrained-runtime evidence; not production deployment or release provenance |
| Headless Playwright against the authoritative ASGI app on `127.0.0.1:18081` | Authenticated local browser evidence rendered the Overview and J.A.R.V.I.S. Observe/Assist routes, submitted `observe system status`, and displayed a durable run, verified evidence, and the explicit no-legacy-availability disclosure. Artifacts: `output/playwright/zasi-javis-authenticated.png` and `output/playwright/zasi-javis-conversation.png` | Fresh local cockpit runtime evidence; no public ingress, external write, hardware, self-evolution, or ASI/AGI evidence |
...
| PR #48 hosted checks for exact merged source head `510e36e59648d58f535c6af09fcac4977da52419` | Actions/JavaScript-TypeScript/Python analysis, Python 3.11/3.12, syntax/type, distribution, Docker image, Docker Build Check, and CodeQL checks passed after multimodal parser-memory, format, evidence-contract, and rejection-envelope hardening; PR-only GHCR publication was skipped. PR #48 merged at `e03de116d7a7d2f6f85c2ae4ecd9469f935a6b86`. | Current merged hosted CI evidence; not release approval |
| PR #50 hosted checks for exact merged source head `9e8068f09a143c66a7fab42d951482f2accd56d9` | Actions/JavaScript-TypeScript/Python analysis, Python 3.11/3.12, syntax/type, distribution, Docker image, Docker Build Check, and CodeQL checks passed for bounded GLB/glTF evidence and authenticated viewer changes; PR-only GHCR publication was skipped. PR #50 merged at `0158e04aee49571493a0b3cfadbb180642cf255a`. | Current merged hosted CI evidence; not release approval |
| PR #51 hosted checks for exact merged documentation head `a1460bc9decf1795e87611b0b93cdba6694bcd18` | Hosted CodeQL, Python 3.11/3.12, syntax/type, distribution, Docker image, Docker Build Check, and Actions/JavaScript-TypeScript checks passed for the glTF evidence reconciliation. PR #51 merged at `a073ee819582aaa1d3f2117327b7301c15f84a23`. | Current merged hosted CI evidence; not release approval |
| Fresh ASGI smoke against the shared host PostgreSQL/Redis services on 2026-09-03 UTC | `backend.app:create_app` started on a temporary loopback port; `/health/live` and `/health/ready` returned `200`, authenticated session bootstrap returned `201`, current-session/profile returned `200`, readiness reported PostgreSQL schema 11 and Redis `ready`, and the process was stopped after the probe. | Local runtime evidence only; not staging or production deployment |
| `docker build --pull=false -t zasi-validation:20260903-2138 .` plus hardened container smoke | Image digest `sha256:7ecb3ed06d03ddd6d82f76b421c903dead2c056087881cf75877906aac323e6f`; readiness returned `200`/healthy with UID `10001:10001`, read-only root filesystem, dropped capabilities, no-new-privileges, bounded memory/PID limits, and private data volume mode `700`. | Local container identity/readiness evidence only; not signed release provenance |
| Fresh isolated package, SBOM, and signature bundle on 2026-09-03 UTC | `python3 -m build` produced wheel/sdist; CycloneDX SBOM generation resolved 362 components; `SHA256SUMS` and four detached signatures (wheel, sdist, SBOM, and checksum manifest) were independently verified with the configured signing identity. Output remained outside the repository. | Local supply-chain/artifact evidence only; not hosted release provenance |
| `npm ci --ignore-scripts`; `npm ls react-router react-router-dom --depth=0`; `npm audit --offline --omit=dev --audit-level=moderate --json` | Lockfile installation resolved React Router DOM `7.18.3`; the local npm audit cache reported zero info/low/moderate/high/critical findings. The online registry audit timed out in this environment and is not counted as fresh external evidence. | Local dependency alignment/audit-cache evidence; refresh online before release |
