...
| Knowledge/RAG | `cvsz/zknowbase` | `api-service` | `assessed` | Source verified; bounded ARIN adapter/runtime foundation is separately `verified` as `ARIN-MIG-0008`; ADR accepted; canonical promotion still requires exact-head compatibility/rollback evidence and an explicit prior-path deprecation decision. |
...
| `ARIN-MIG-0008` | ARIN ↔ zknowbase bounded integration | `cvsz/zknowbase@c71da3da3277d3cdd5f37435b7274c6f8f595946` | `backend/arin/knowledge.py`, `backend/arin/knowledge_write.py`, `backend/arin/knowledge_runtime.py` | `verified` | PRs #115, #122-#125 verify contract/read transport/provenance/write-policy/degraded behavior; ADR-ARIN-0003 accepted. PR #168 inventoried consumers. PR #169 exact head `c6b2761dc9ca14e438ea7fb053ee4f8a70db6444` passed all nine required gates and merged signed as `bd939aa95caeaf63783678b10d824410a03bbd5e`, verifying the additive server-side runtime consumer. Canonical promotion remains blocked until compatibility/rollback regressions pass exact-head CI and the prior-path deprecation decision is explicit; write transport remains absent. |
