...
| Knowledge/RAG | `cvsz/zknowbase` | `api-service` | `assessed` | Source capability verified at `c71da3da3277d3cdd5f37435b7274c6f8f595946`; bounded ARIN read transport and provenance mapping are integrating and remain non-canonical until exact-head evidence and ADR requirements pass |
...
| `ARIN-MIG-0008` | ARIN ↔ zknowbase read-contract foundation | `cvsz/zknowbase@c71da3da3277d3cdd5f37435b7274c6f8f595946` | `backend/arin/knowledge.py` | Pending integration ADR before production enablement | `integrating` | PR #115 contract foundation and PR #122 read transport passed exact-head gates; current provenance slice requires its own exact-head evidence | Disable/remove ARIN knowledge client; no write scope, data migration, credential store, or source copy | Read-only search/query only; tenant/scoped key/bounded timeout enforced; zknowbase citations are mapped into tenant-bound evidence and malformed/cross-tenant provenance fails closed; physical actuation unaffected |
