...
| Knowledge/RAG | `cvsz/zknowbase` | `api-service` | `assessed` | Source capability verified at `c71da3da3277d3cdd5f37435b7274c6f8f595946`; bounded ARIN read transport/provenance and a fail-closed write authorization gate are integrating and remain non-canonical until exact-head evidence and ADR requirements pass |
...
| `ARIN-MIG-0008` | ARIN ↔ zknowbase integration | `cvsz/zknowbase@c71da3da3277d3cdd5f37435b7274c6f8f595946` | `backend/arin/knowledge.py`, `backend/arin/knowledge_write.py` | Pending integration ADR before production enablement | `integrating` | PR #115 contract foundation, PR #122 read transport, and PR #123 provenance mapping passed exact-head gates; current write-policy-gate slice requires its own exact-head evidence | Disable/remove ARIN knowledge clients; no data migration, credential store, or source copy | Read-only search/query remains tenant/scoped/bounded; write transport is still absent and future mutation construction now requires explicit `knowledge:write` scope, affirmative policy, tenant context and approval evidence; physical actuation unaffected |
