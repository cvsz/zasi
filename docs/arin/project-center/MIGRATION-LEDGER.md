...
| Bounded tools/MCP | `cvsz/zcoder` | `api-service` / narrow adapter | `assessed` | Source is pinned and MIT licensed, but remains `partial` because reproducible exact-head containment/MCP CI/security evidence is not attached; ARIN adapter is still missing |
...
| `ARIN-MIG-0005` | Bounded tools/MCP source assessment | `cvsz/zcoder@7153e8b7a48e9d7f9f1976fae2b23f270ac44dab` | future bounded ZASI tool adapter | Pending integration ADR | `assessed` | Source commit GitHub-signature verified and MIT licensed; source contains pinned CodeQL/release/container/SBOM workflow dependencies, but the available exact-SHA PR-workflow query returned no runs | No runtime dependency/source copy added; remove evidence-only assessment without runtime/data mutation | Source remains `partial`; do not promote containment/MCP or ARIN integration until reproducible deny-by-default evidence exists |
