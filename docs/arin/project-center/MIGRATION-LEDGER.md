...
| Knowledge/RAG | `cvsz/zknowbase` | `api-service` | `assessed` | Source verified; bounded ARIN adapter foundation is separately `verified` as `ARIN-MIG-0008`; ADR accepted; consumer inventory recorded; canonical promotion still requires a real consumer migration plus compatibility/rollback evidence. |
...
| `ARIN-MIG-0008` | ARIN ↔ zknowbase bounded integration | `cvsz/zknowbase@c71da3da3277d3cdd5f37435b7274c6f8f595946` | `backend/arin/knowledge.py`, `backend/arin/knowledge_write.py` | `verified` | PRs #115, #122-#125 verify contract/read transport/provenance/write-policy gate/degraded behavior. ADR-ARIN-0003 is accepted. Task 5 consumer inventory found no separate production ARIN runtime consumer to mark migrated; write transport remains absent. Canonical promotion stays blocked on a real consumer migration plus compatibility/rollback evidence. |
