...
| Bounded tools/MCP | `cvsz/zcoder` | `api-service` / narrow adapter | `candidate` | Source pinned at `7153e8b7a48e9d7f9f1976fae2b23f270ac44dab`; Task 6 security requirements, capability descriptors, and denial-policy regressions have exact-head ZASI evidence through PR #128. `ADR-ARIN-0002` now proposes a service-first adapter; full source assessment and ADR acceptance remain required before runtime dependency |
...
| `ARIN-MIG-0005` | Bounded tools/MCP source assessment | `cvsz/zcoder@7153e8b7a48e9d7f9f1976fae2b23f270ac44dab` | future bounded tool adapter | `ADR-ARIN-0002` proposed | `candidate` | Task 6 native ARIN boundary evidence exists through PR #128; full source assessment/exact-head ZCoder evidence remains incomplete | No runtime dependency/source copy | No runtime authority added |
...
| `ARIN-MIG-0012` | ZCoder service-first adapter decision | `cvsz/zcoder@7153e8b7a48e9d7f9f1976fae2b23f270ac44dab` | future ARIN bounded tool adapter | `ADR-ARIN-0002` | `candidate` | ADR PR exact-head CI/security evidence required before acceptance | Revert ADR; current native descriptor/policy-only state remains | ADR proposes disabled-by-default API/narrow-adapter integration, fake/local contract transport first, and explicitly prohibits raw shell/MCP/actuator passthrough |
