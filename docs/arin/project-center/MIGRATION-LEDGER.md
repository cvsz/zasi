...
| `ARIN-MIG-0016` | Legacy JARVIS stable-route regression contract | `cvsz/zasi@04760b42f4f46ad4950e2982e444b713e99872e3` | `tests/test_api.py` | `verified` | PR #137 exact head `04760b42f4f46ad4950e2982e444b713e99872e3` passed all nine required gates before merge. Both `/api/jarvis/chat` and `/api/jarvis/stream` remain HTTP 410; no endpoint or alias execution authority was enabled. |
| `ARIN-MIG-0017` | ARIN identity compatibility regression tests | `cvsz/zasi@086bc8a030b69043b46fbd11501013ac83ad8f2a` | `tests/test_arin_identity_compatibility.py` | `verified` | 22 regression tests cover all 8 JARVIS/ZARVIS stable machine identifiers; all pass. Merged via `arin/task3-identity-regression-tests`; alias authority remains isolated. |
| `ARIN-MIG-0018` | Tenant/session schema isolation tests | `cvsz/zasi@2bc5a53` | `tests/test_arin_session_schema.py` | `verified` | 19 tests cover session tenant isolation, authentication tenant matching, and incompatible payload rejection. |
| `ARIN-MIG-0019` | Versioned canonical contracts (Session, Plan, Approval) | `cvsz/zasi@beb3488` | `src/control_plane/contracts/contracts.py` | `verified` | Versioned v1.0.0 contracts use StrictModel (`extra='forbid'`) to reject incompatible payloads; 33 contract validation tests pass. |
| `ARIN-MIG-0020` | Canonical v1 OpenAPI/client artifact generator | `cvsz/zasi@1caebc064cbd644770ae5bc2503fad6c5c52415a` | `scripts/generate_arin_contract_artifacts.py`, `tests/test_arin_contract_artifacts.py` | `verified` | PR #141 exact head passed all nine required gates. Generator derives schema-only OpenAPI 3.1 components and dependency-free TypeScript types; no runtime route, provider credential, live tool authority, infrastructure dependency or actuator path admitted. |
| `ARIN-MIG-0021` | Canonical v1 backward-compatibility regressions | `cvsz/zasi@c5114c33076328cc94d0f6aee2c005fae76eafae` | `tests/test_arin_contract_compatibility.py` | `verified` | PR #145 exact head produced nine required workflow runs and merged as signed commit `23a57fbe7fd2c1b3e1d5c7c40284b53e796a7aac`. Tests preserve omitted-version and explicit v1.0.0 payload compatibility while unsupported future versions and authority-expanding unknown fields fail closed. Test-only; no runtime/actuator authority added. |
