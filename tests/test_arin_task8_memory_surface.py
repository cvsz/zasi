...
class MemorySurfaceTests(unittest.TestCase):
    ...
    def test_mutations_require_the_authenticated_token(self) -> None:
        self.assertRegex(
            self.surface,
            r"api\.post\(\s*['\"]/api/v2/memory['\"]\s*,\s*token\s*,",
            "memory creation must use the authenticated token",
        )
        self.assertRegex(
            self.surface,
            r"api\.request\(\s*`/api/v2/memory/\$\{memoryId\}`\s*,\s*\{\s*token\s*,\s*method:\s*['\"]DELETE['\"]",
            "memory deletion must use the authenticated token",
        )
